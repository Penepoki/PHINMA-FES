...
from django.contrib.auth import get_user_model
from hrapp.utils.user_utils import get_full_name
...
import json
from .utils.generate_evaluation_pdf import generate_evaluation_pdf

@csrf_exempt  # For demonstration; use proper authentication/CSRF in production
def save_evaluation_pdf(request):
    ...
    if professor_id:
        try:
            professor_user = User.objects.get(id=professor_id)
            professor_name = get_full_name(professor_user)
        except User.DoesNotExist:
    ...
    # Build filename: ProfessorName_Semester_COPUSNumber_Date.pdf
    safe_professor_name = professor_name.replace(' ', '')
    semester = data.get('semester', 'Semester').replace(' ', '')
    copus_number = data.get('copus_number', data.get('evaluation_type', 'COPUS')).replace(' ', '')
    date = data.get('date', 'date')
    filename = f"{safe_professor_name}_{semester}_{copus_number}_{date}.pdf"

    response = HttpResponse(pdf_bytes, content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    response['X-Suggested-Filename'] = filename  # Custom header for frontend to use
    return response
