  ...
  const renderAnimatedButton = (key: string, label: string) => {
    ...
    return (
      <button
        ...
        className={`relative flex h-8 items-center justify-center rounded-xl transition-all duration-300 ${isActive ? "scale-125 ring-2 ring-white" : ""} `}
      >
        ...
        <span
          className={`transform px-1 text-center text-xs font-medium text-white transition-all ${isActive || isHovering ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"} block md:absolute md:inline md:transition-all md:duration-300`}
        >
          {label}
        </span>
      </button>
